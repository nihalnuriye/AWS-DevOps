...
#(ssh-add C:/Users/sarum/.ssh/first.key.pem)******
# 9. Connect to the "Wordpress-Bastion Host" instance in "public-1b" subnet.
ssh -A ec2-user@54.197.205.123 (Public IP of Wordpress-Bastion Host) # Don't forget to change the IP with your instance IP.

# 10. connect to the Database instance in the "private-1b" subnet.
ssh ec2-user@ 10.7.2.11(Private IP of Database Instance) # Don't forget to change the IP with your instance IP.
...
ping 10.7.5.108 (Private IP of Private Instance) # Don't forget to change the IP with your instance IP. 
